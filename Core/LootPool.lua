...
local PERSISTED_CACHE_VERSION = 24 -- bumped: mount IDs and housing items filtered from persisted entries
...
local _seasonDungeonByInstanceID = {} -- { [WoW instanceID] = ejInstanceID }  (populated from SeasonData.dungeons[id].mapID)
local _seasonRaidIDs = {} -- set: { [ejInstanceID] = true }
local _seasonRaidByMapID = {} -- { [WoW mapID] = ejInstanceID }  (populated from SeasonData.raidInstanceIDs[id].mapID)
local _seasonRaidByName = {} -- { [localizedName] = ejInstanceID }  (fallback for old SeasonData without mapID)
...
function LootPool.BuildSeasonFilter()
    ...
    wipe(_seasonDungeonByInstanceID)
    wipe(_seasonRaidIDs)
    wipe(_seasonRaidByMapID)
    wipe(_seasonRaidByName)
    ...
    for _, instanceID in ipairs(sd.dungeonInstanceIDs or {}) do
        ...
        -- Map WoW InstanceID → EJ instanceID when SeasonData includes mapID.
        local dungeonData = sd.dungeons and sd.dungeons[instanceID]
        if dungeonData and dungeonData.mapID then
            _seasonDungeonByInstanceID[dungeonData.mapID] = instanceID
        end
    end

    -- Populate raid instance IDs from static data.
    for instanceID, raidEntry in pairs(sd.raidInstanceIDs or {}) do
        _seasonRaidIDs[instanceID] = true
        -- raidEntry may be true (legacy) or { mapID=N } (new gen format).
        local mapID = type(raidEntry) == "table" and raidEntry.mapID or nil
        if mapID then
            _seasonRaidByMapID[mapID] = instanceID
        end
    end
    ...
    if numTiers > 0 then
        ...
        -- Fallback for old SeasonData (no mapID on raids): build a name→EJ ID
        -- map by scanning the EJ raid list.  Skipped once SeasonData is
        -- regenerated with mapID values.
        if not next(_seasonRaidByMapID) then
            idx = 1
            while true do
                local instanceID, name = EJ_GetInstanceByIndex(idx, true)
                if not instanceID then
                    break
                end
                if _seasonRaidIDs[instanceID] and name then
                    _seasonRaidByName[name] = instanceID
                end
                idx = idx + 1
            end
        end
...
-- Returns the EJ instanceID for a dungeon given the WoW InstanceID that
-- GetInstanceInfo() returns as its 8th value.  More reliable than name lookup
-- because it is locale-independent and requires no EJ scan at runtime.
-- Returns nil when SeasonData has not been regenerated with mapID values yet.
function LootPool.GetSeasonDungeonByInstanceID(instanceID)
    EnsureSeasonFilter()
    return _seasonDungeonByInstanceID[instanceID]
end
...
-- Returns the EJ instanceID for a raid given the WoW mapID that
-- GetInstanceInfo() returns as its 8th value.  Requires SeasonData regenerated
-- with mapID values (VoidcoreAdvisorGen 2.x+).  Falls back to name lookup for
-- old SeasonData.  Returns nil when the raid is not a current-season raid.
function LootPool.GetSeasonRaidByMapID(mapID)
    EnsureSeasonFilter()
    return _seasonRaidByMapID[mapID]
end

-- Name-based fallback for old SeasonData (before mapID support).
function LootPool.GetSeasonRaidByName(name)
    EnsureSeasonFilter()
    return _seasonRaidByName[name]
end
